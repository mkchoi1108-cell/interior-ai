...
export async function POST(req: NextRequest) {
  try {
    ...
    // Step 1: Gemini 2.0 Flash analyzes room
    const analysisParts = [
...
JSON으로만 응답 (다른 텍스트 없이):
{
  ...
  "styleDescription": "스타일 설명 (한국어 2-3문장)"
    ...
    // Step 2: Gemini 2.0 Flash generates interior image (free tier 지원)
    const imageResponse = await ai.models.generateContent({
      model: 'gemini-2.0-flash-preview-image-generation',
      contents: [{
        role: 'user',
        parts: [{ text: `Generate a photorealistic interior design photo: ${analysis.imagePrompt}. Professional architectural photography, well-lit, high quality, Architectural Digest style.` }],
      }],
